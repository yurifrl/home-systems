...
| 4 | Restore ingestion — resolve stuck Longhorn expansion (`volume-head-001.img already exists` on tp1 replica) | `[TODO]` | Longhorn (tp1 replica) | BLOCKED (`el5.1`, P1) — needs user decision: (A) recreate volume fresh at 10Gi (loses ~30d single-replica history) or (B) restart longhorn instance-manager on tp1 (blast radius) |
