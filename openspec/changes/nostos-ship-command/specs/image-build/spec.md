...
- **WHEN** operator runs `nostos ship rpi01 --output /path/to/image.raw`
...
- **WHEN** operator runs `nostos ship dell02 --output /path/to/image.raw`
- **THEN** system produces a raw disk image containing:
...
- **WHEN** operator runs `nostos ship rpi01 --device /dev/disk10`
...
- **WHEN** `nostos ship <node>` is invoked
...
- **WHEN** `nostos ship <node>` is invoked and Tailscale OAuth credentials cannot be resolved
...
#### Scenario: Dry run
- **WHEN** operator runs `nostos ship rpi01 --dry-run`
