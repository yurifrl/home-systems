...
**Goals:**
- One command (`nostos flash <node>`) produces a flashable image with all config embedded
...
**Decision**: `nostos flash` mints a Tailscale auth key (via OAuth) and embeds it in the machineconfig within the image. Key is single-use, pre-authorized, tagged.
...
- **[Pre-minted key expiry]** → If image isn't used within 90 days, key expires. Mitigation: document this; `nostos flash` warns about expiry window.
